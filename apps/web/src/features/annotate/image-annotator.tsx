...
export interface ImageAnnotatorProps {
  ...
  onCancel: () => void;
  /** Lets the caller offer "send without editing" if the preview fails to load. */
...
/** How long a silent decode hang gets before we give up and show the error state. */
 ...
 */
export function ImageAnnotator({ file, onDone, onCancel, onSkip }: ImageAnnotatorProps) {
  ...
  useEffect(() => {
    ...
    // Large, unprocessed camera-capture files decode more slowly (or, on some
    // browsers, silently never fire onload) than already-reasonable gallery
    // picks — without an onerror handler and a timeout, this left the canvas
    // permanently blank with "Done" disabled and no way forward but Cancel.
    const timeout = window.setTimeout(() => setStatus((s) => (s === 'loading' ? 'error' : s)), DECODE_TIMEOUT_MS);
    image.onload = () => {
  ...
  return (
    <Modal open onClose={onCancel} title="Edit photo">
      <div className="flex max-h-[70vh] flex-col gap-3">
        <div className="flex min-h-48 items-center justify-center overflow-hidden rounded-xl border border-border bg-surface-sunken">
          {status === 'loading' && (
            <div className="flex flex-col items-center gap-2 p-8 text-sm text-fg-muted">
              <span
                aria-hidden
          ...
          {status === 'error' && (
            <div className="flex flex-col items-center gap-3 p-8 text-center text-sm text-fg-muted">
              <p>Couldn't preview this photo.</p>
              {onSkip && (
                <Button type="button" size="sm" onClick={() => onSkip(file)}>
          ...
          <canvas
            ref={canvasRef}
            className={`max-h-[50vh] max-w-full touch-none ${ready ? '' : 'hidden'}`}
